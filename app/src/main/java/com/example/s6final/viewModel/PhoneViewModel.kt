package com.example.s6final.viewModel
...
import com.example.s6final.modelo.PhoneRepository
import com.example.s6final.modelo.local.database.PhoneRoomDatabase
...
class PhoneViewModel (application: Application): AndroidViewModel(application){
    ...
    init{
        val bd=
            PhoneRoomDatabase.getDataBase(application) //Instanciar la base de datos  (para acceder a ella)
        ...
        viewModelScope.launch {
            ...
            repository.fechPhone()// esta es una función que viene del PhoneRepository, antes hemos instanciado el Repositorio como repository
        }
    }
