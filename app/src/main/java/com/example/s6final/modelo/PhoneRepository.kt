...
import com.example.s6final.modelo.remote.apiRetrofit.RetrofitClient
...
class PhoneRepository (private val phoneDao: PhoneDao){
    ...
    suspend fun  fechPhone(){ // suspend fun es de courrutines
        ...
        service.onSuccess {
            ...
            when(it.code()){
                in 200..299-> it.body()?.let {
                    ...
                    phoneDao.insertAllPhone(fromInternetToPhoneEntity(it)) // viene del Dao ( insertall) y viene del mapper (fromInternetToPhoneEntity)
    ...
    suspend fun fetchPhoneDetail(id: String): DetailsPhoneEntity? {
        ...
        return service.getOrNull()?.body()?.let { phoneDetail ->
            ...
            phoneDao.insertPhoneDetail(phoneDetailEntity) //la función viene del PhoneDao  :  suspend fun insertPhoneDetail(course: DetailsShoesEntity)
            phoneDetailEntity
        }
    }
